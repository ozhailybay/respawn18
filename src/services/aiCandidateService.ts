...
export function preFilterCandidates(
  ...
  return applications.filter(application => {
    ...
    const userData = application.student as unknown as UserData & { skills?: string[] };
    if (userData.skills && userData.skills.length > 0) {
      return userData.skills.some(skill => {
        const skillName = typeof skill === 'string' ? skill : skill.name;
        return internship.skills.some(requiredSkill => 
          requiredSkill.toLowerCase().includes(skillName.toLowerCase()) ||
          skillName.toLowerCase().includes(requiredSkill.toLowerCase())
        );
      });
    }
  ...
  try {
    ...
    const model: GenerativeModel = genAI.getGenerativeModel({
      ...
      messages: [
        {
          role: 'system',
          content: `You are an expert HR AI assistant for Respawn, an educational platform that connects students with micro-internships.
...
- Skills: ${userData?.skills?.length ? userData.skills.map(skill => skill.name).join(', ') : 'Not specified'}
- Education: ${userData?.education?.length ? userData.education.map(edu => edu.degree).join(', ') : 'Not specified'}
- Experience: ${userData?.experience?.length ? userData.experience.map(exp => exp.title).join(', ') : 'Not specified'}
...
- Years of Experience: ${userData?.experience?.length ? userData.experience.length : 0}
- Bio: ${userData?.bio || 'Not specified'}
- Level in platform: ${application.student?.level || 1}
...
`
    ...
    const candidate = {
      ...
      skills: userData?.skills?.map(skill => skill.name) || [],
      education: userData?.education?.map(edu => edu.degree).join(', ') || '',
      experience: userData?.experience?.map(exp => exp.title).join(', ') || '',
      bio: userData?.bio || '',
      university: userData?.university || '',
      major: userData?.major || '',
      graduationYear: userData?.graduationYear || '',
      yearsOfExperience: userData?.experience?.length || 0,
    };
    
    // Create detailed prompt for Gemini
    const prompt = `
You are an expert HR AI assistant for Respawn, an educational platform that connects students with micro-internships.
Your task is to evaluate how well a candidate matches a micro-internship opportunity.
