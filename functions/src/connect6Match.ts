...
async function calculateMatchScore(
  ...
  // 1. Skills match (35%)
  const userSkills = userData.skills?.map((s: any) => s.name || s) || [];
  ...
  const userExperienceYears = userExperience.reduce((total: number, exp: any) => {
...
function diversifyResults(matches: Connect6MatchResult[]): Connect6MatchResult[] {
  ...
  const result = [...diversifiedResults];
  let i = 0;
 ...
 */
export const connect6Match = onCall<Connect6MatchRequest>({
  maxInstances: 10,
  memory: '1GiB',
  timeoutSeconds: 60,
}, async (request) => {
 ...
 */
export const connect6MatchExplain = onCall({
  maxInstances: 10,
  memory: '512MiB',
  timeoutSeconds: 30,
}, async (request) => {
 ...
 */
export const connect6BatchUpdate = onCall({
  maxInstances: 5,
  memory: '512MiB',
  timeoutSeconds: 60,
}, async (request) => {
  ...
  try {
    ...
    // Create a pub/sub message for each user
    // Use messaging instead of pubsub since pubsub is not available in firebase-admin
    const messaging = admin.messaging();
    
    const publishPromises = targetUserIds.map(async (userId: string) => {
      // Instead of pubsub, we'll use a Firestore document to trigger the update
      const updateRef = db.collection('connect6-updates').doc();
